...
- **Fallas — evidencia por etapa** (Sesión 24): enum `EtapaFotoFalla {reporte|resolucion}` + columna `etapa` en `FotoFalla` (default `reporte`, migración aditiva); `subir` acepta/valida `etapa` (400 si inválida) y las fotos heredadas del punto quedan `reporte`; `FallaDetallePage` muestra dos galerías (📸 al reportar / 🔧 al resolver) con permisos por estado (`puedeEvidenciaResolucion`); PDF separa en dos bloques omitiendo el vacío. Verificado por API + `npm run build`.
- **Registro de fallas Fase 3 — analítica y exportación** (Sesión 25): servicio `fallasEstadisticasService.calcularEstadisticas` (un `findMany` filtrado + agregación en JS: KPIs, MTTR, datasets por categoría/severidad/origen/componente/modelo/producto, tendencia mensual, embudo, `detalle` plano). 3 endpoints **antes de `/:id`** en `routes/fallas.js`: `GET /api/fallas/estadisticas`, `/reporte.pdf` (PDF resumen HYDRA con primitivas nuevas `hbarChart`/`pieChart` en `pdf/ui.js`), `/export.xlsx` (dep `exceljs`, hojas Resumen+Detalle). Frontend: dep `recharts`, `FallasDashboardPage` (filtros → query params; KPIs, 6 gráficas en `components/fallas/FallasGraficas.jsx`, tabla, export), enlace 📊 desde `FallasPage`, e histórico por producto desde `FlotaPage` (`/fallas/dashboard?productoId=…`, sembrado vía `useSearchParams`). Verificado: 3 endpoints → 200 + filtros, `/:id` sin regresión, `npm run build`.
- **Panel de control** (Sesión 26): hub admin en `/panel` con 4 tarjetas (Formatos, Usuarios, Modelos, Categorías de falla). Header reducido a `Órdenes · Flota · Fallas · Productos · ⚙ Panel`. Gate `puedePanel = esSuper || rol === 'gerente_soporte'`. Sin schema, sin endpoints nuevos. Ingeniero de soporte pierde descubrimiento UI de esos catálogos (backend sin endurecer). Verificado por `npm run build`.
- **Perfil de usuario** (Sesión 27–28): `fotoUrl`, `distintivo` (callsign único, ≤ 16 chars upper) y `descripcionPuesto` en `Usuario` (migración aditiva a mano). Página `/mi-perfil` para auto-edición. 4 endpoints (`GET/PATCH /api/usuarios/me`, `POST/DELETE /api/usuarios/:id/foto`) con multer 2MB JPG/PNG/WebP + middleware `requireDueñoOGerente` (JWT usa `req.user.sub`). Componente `<Avatar />` (`components/ui.jsx`, foto o iniciales fallback) en Header, cards de personal de O/T (Cierre/Inspeccion/FallaDetalle; CrearOT usa `<select>` nativo → N/A), firmas UI del cierre (`showCallsign`) y bloque de firmas del PDF HYDRA de O/T + falla (foto circular vía `signatureCard(doc,…,buffers)` + callsign mono color acento). Selects de orden/falla extendidos con `fotoUrl`/`distintivo` (`INCLUDE_USUARIO_BASICO`, `INCLUDE_FALLA`); buffers cargados con `storage.getBuffer` filtrando `esImagenEmbebible` y tolerando fallos. Verificado: smoke API (`/me` con campos nuevos, PDF de O/T cerrada → `%PDF` 18MB sin 500) + `npm run build`. Pendiente: smoke UI visual manual por rol. **Limitación conocida:** pdfkit solo embebe PNG/JPEG; una foto de perfil subida en WebP se muestra en el Avatar web pero cae a iniciales en el PDF (intencional, documentado en `EXT_IMG_VALIDAS` de ambos `pdfController`).
